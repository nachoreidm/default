...
import { ALLOWED_PAIRS, RISK_LIMITS, CONFIDENCE_MAX_SIZE_PCT, TAKE_PROFIT_RR_MULTIPLE, MOMENTUM_THRESHOLD_PCT, MOMENTUM_ONLY_MAX_CONFIDENCE, } from "./types.js";
...
server.tool("portfolio_check_stops", `Check every open paper position against its stop-loss and take-profit using live prices, and auto-close any that have breached either. Take-profit is fixed at ${TAKE_PROFIT_RR_MULTIPLE}:1 risk/reward above entry, set automatically when the position was opened. Intended to be called both on demand and from a scheduled monitoring run, so stops and profit-taking are respected even when nobody is actively chatting with the agent.`, {}, async () => wrap(() => checkStops())());
