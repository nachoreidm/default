...
import { hasReachedOneR, trailingStopCandidate, historicalPeakSinceEntry } from "./trailing-math.js";
import { ALLOWED_PAIRS, RISK_LIMITS, CONFIDENCE_MAX_SIZE_PCT, TAKE_PROFIT_RR_MULTIPLE, MOMENTUM_ONLY_MAX_CONFIDENCE, isAllowedPair, } from "./types.js";
...
async function liveCashEur() {
    ...
    if (raw === undefined) {
        ...
        throw new KrakenApiError(`Kraken Balance response has assets (${Object.keys(balance).join(", ")}) but no ZEUR/EUR key - the assumed EUR balance key may be wrong. Verify before trusting any sizing math.`);
    }
    return Number(raw);
}
...
async function formatVolume(pair, n) {
...
async function formatPrice(pair, n) {
...
export async function openPosition(input) {
    ...
    const entryOrder = await addOrder({
        pair: pairCode(input.pair),
        type: "buy",
        ordertype: "market",
        volume: await formatVolume(input.pair, estVolume),
    });
    ...
    const stopOrder = await addOrder({
        ...
        type: "sell",
        ordertype: "stop-loss",
        price: await formatPrice(input.pair, input.stop_loss),
        volume: await formatVolume(input.pair, quantity),
    });
...
async function recordClose(state, pos, exitPrice, exitFee, reason) {
    ...
    for (const pos of [...state.open_positions]) {
        ...
        if (pos.stop_order_txid) {
            ...
            if (stopOrder && stopOrder.status === "closed") {
                const closed = await recordClose(state, pos, Number(stopOrder.price), Number(stopOrder.fee), pos.trailing_active
        ...
        if (!pos.trailing_active) {
            if (ticker.last >= pos.take_profit) {
                ...
                if (sellTxid) {
                    const filled = await pollForFill(sellTxid);
                    const closed = await recordClose(state, pos, Number(filled.price), Number(filled.fee), `Take-profit auto-triggered (price ${ticker.last} >= target ${pos.take_profit}, ${TAKE_PROFIT_RR_MULTIPLE}:1 risk/reward).`);
        ...
        if (pos.trailing_active) {
            ...
            if (candidate > pos.stop_loss && pos.stop_order_txid) {
                ...
                if (!newStopTxid) {
                    ...
                    throw new KrakenApiError(`CRITICAL: cancelled stop order ${pos.stop_order_txid} for position ${pos.id} (${pos.pair}) to raise the trailing floor to ${candidate}, but the replacement order failed - Kraken response: ${JSON.stringify(newStopOrder)}. This position has NO stop-loss protection right now. Check Kraken directly immediately.`);
                }
