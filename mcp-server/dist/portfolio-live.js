...
async function liveCashEur() {
    ...
    if (raw === undefined) {
        throw new KrakenApiError(`Kraken Balance response has no ZEUR/EUR key - got: ${Object.keys(balance).join(", ") || "(empty)"}. Verify the real balance key before trusting any sizing math.`);
    }
    return Number(raw);
}
