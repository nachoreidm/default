...
- Crypto only, on purpose: Kraken xStocks (tokenized equities) were tried
  ...
  restriction, not a rollout gap), so they could never go live from this
  account even though public market data for them worked fine in paper
  trading. Don't re-add xStocks pairs without the user explicitly revisiting
  that.
